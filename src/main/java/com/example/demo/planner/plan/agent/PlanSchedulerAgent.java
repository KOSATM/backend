...
public class PlanSchedulerAgent {
    ...
    public String createTravelPlan(List<DayPlanResult> dayPlans, Map<String, Object> arguments) {
        ...
        String systemPrompt = """
                ...
                입력된 placeId 목록만 사용하여 일정의 순서(order)와 시간(start/end)을 결정해야 합니다.

                여행지의 상세 정보(name, category, address 등)는 모두 백엔드에서 placeId로 매핑되므로,
                출력에는 절대 name이나 기타 정보가 포함되지 않습니다.
                출력은 오직 placeId 기반 일정 JSON만 포함해야 하며,
                ...
                1) 여행지 추가/삭제 금지
                - 입력된 placeId만 사용해 순서(order)와 시간(start/end)만 결정합니다.
                ...
                - 입력된 placeId만 그대로 사용합니다.

                6) 기본 일정 시작 시간
                - 일반 날짜(dayIndex = 2 ~ totalDays - 1)는 10:00 시작을 기본으로 합니다.

                ------------------------------------------------------------
                [도착일(dayIndex = 1) 체크인 규칙 - 현실적인 버전]

                - 호텔 체크인 시간은 15:00으로 가정합니다.
                - 도착일의 관광 일정은 반드시 **15:00 이후**에 시작해야 합니다.
                - 체크인 이전에는 관광 일정을 배치하지 않습니다.
                - 도착일은 1~3개의 가벼운 일정만 구성합니다.
                - 장거리 이동·장시간 활동(120분 이상) 금지.

                ------------------------------------------------------------
                [출발일(dayIndex = totalDays) 체크아웃 규칙]

                - 체크아웃 시간은 11:00으로 가정합니다.
                - 출발일(dayIndex = totalDays)의 일정은 **11:00 이후에만** 시작해야 합니다.
                - 출발일은 부담 없는 1~3개의 일정으로 구성합니다.
                - 장시간 활동(120분 이상) 금지.
                - 일정 종료 시간은 **15:00~17:00 사이**여야 합니다.
                ...
                ------------------------------------------------------------
                [출력(JSON) 형식 - 제목 제거된 버전]

                반드시 아래 구조처럼 출력해야 하며 JSON ONLY로 출력합니다:
                ...
                현실적이고 자연스러운 여행 일정을 생성하세요.
                        ...
                        """;
        String userPrompt = buildLLMJsonInput(dayPlans, startDate);
